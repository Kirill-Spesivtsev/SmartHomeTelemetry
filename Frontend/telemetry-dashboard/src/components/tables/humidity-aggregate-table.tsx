...
export default function HumidityAggregateTable({ rows } : { rows: AirQualityAggregate[] }) {
  return (
          ...
          <tbody>
            {rows.map(
              (row: AirQualityAggregate) => (
                <tr key={`${row.locationName}-hum`} className="border-t border-slate-800/80">
                  <td className="px-3 py-2 font-medium text-slate-200">{row.locationName}</td>
                  <td className="px-3 py-2">{row.count}</td>
                  <td className="px-3 py-2">{row.avgHumidity.toFixed(2)}</td>
                  <td className="px-3 py-2">{row.minHumidity}</td>
                  <td className="px-3 py-2">{row.maxHumidity}</td>
                </tr>
              ),
            )}
          </tbody>
